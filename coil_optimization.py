...
LENGTH_WEIGHT = 1e-8 # change this one to get shorter or longer coils
...
def cb(x):
    k[0] += 1
    np.savetxt(f"qa_iters/iteration_{k[0]:04}.txt", x)
...
################################################################################
""")
res = minimize(fun, dofs, jac=True, method='L-BFGS-B', options={'maxiter': MAXITER, 'maxcor': 300}, tol=1e-15, callback=cb)
